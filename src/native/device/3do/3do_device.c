// 3do_device.c - 3DO console output implementation for Joypad
...
static uint8_t parse_extension_to_router(uint8_t* buffer, size_t buffer_size) {
  ...
  while (offset < buffer_size && count < MAX_PLAYERS) {
    ...
    if ((id_nibble & 0b1100) != 0) {
      ...
      if (byte0 & 0x80) buttons |= JP_BUTTON_B3;  // A → B3
      if (byte0 & 0x40) buttons |= JP_BUTTON_DL;  // Left
      if (byte0 & 0x20) buttons |= JP_BUTTON_DR;  // Right
      if (byte0 & 0x10) buttons |= JP_BUTTON_DU;  // Up
      if (byte0 & 0x08) buttons |= JP_BUTTON_DD;  // Down

      // Byte 1: [Tail1][Tail0][L][R][X][P][C][B]
      uint8_t byte1 = buffer[offset + 1];
      if (byte1 & 0x20) buttons |= JP_BUTTON_L1;  // L
      if (byte1 & 0x10) buttons |= JP_BUTTON_R1;  // R
      if (byte1 & 0x08) buttons |= JP_BUTTON_S1;  // X → Select
      if (byte1 & 0x04) buttons |= JP_BUTTON_S2;  // P → Start
      if (byte1 & 0x02) buttons |= JP_BUTTON_B2;  // C → B2
      if (byte1 & 0x01) buttons |= JP_BUTTON_B1;  // B → B1
    ...
    // Joystick: 3-byte ID header (0x01, 0x7B, 0x08)
      ...
      if (byte7 & 0x80) buttons |= JP_BUTTON_DL;
      if (byte7 & 0x40) buttons |= JP_BUTTON_DR;
      if (byte7 & 0x20) buttons |= JP_BUTTON_DD;
      if (byte7 & 0x10) buttons |= JP_BUTTON_DU;
      if (byte7 & 0x08) buttons |= JP_BUTTON_B2;  // C
      if (byte7 & 0x04) buttons |= JP_BUTTON_B1;  // B
      if (byte7 & 0x02) buttons |= JP_BUTTON_B3;  // A
      if (byte7 & 0x01) buttons |= JP_BUTTON_L2;  // FIRE → L2

      // Byte 8: [Tail:4][R][L][X][P]
      uint8_t byte8 = buffer[offset + 8];
      if (byte8 & 0x08) buttons |= JP_BUTTON_R1;
      if (byte8 & 0x04) buttons |= JP_BUTTON_L1;
      if (byte8 & 0x02) buttons |= JP_BUTTON_S1;  // X
      if (byte8 & 0x01) buttons |= JP_BUTTON_S2;  // P
    ...
    // Mouse: ID 0x49
      ...
      // Buttons
      if (byte1 & 0x01) buttons |= JP_BUTTON_B1;  // Left
      if (byte1 & 0x02) buttons |= JP_BUTTON_B3;  // Middle
      if (byte1 & 0x04) buttons |= JP_BUTTON_B2;  // Right
...
// Uses 3DO button aliases: TDO_BUTTON_A = JP_BUTTON_B3, etc.
static inline void map_usbr_to_3do_joypad(_3do_joypad_report* report, uint32_t buttons)
{
    ...
    //   TDO_BUTTON_A = JP_BUTTON_B3 (top)
    //   TDO_BUTTON_B = JP_BUTTON_B1 (middle)
    //   TDO_BUTTON_C = JP_BUTTON_B2 (bottom)
    //   TDO_BUTTON_L = JP_BUTTON_L1
    //   TDO_BUTTON_R = JP_BUTTON_R1
    //   TDO_BUTTON_X = JP_BUTTON_S1
    //   TDO_BUTTON_P = JP_BUTTON_S2
...
static inline void map_usbr_to_3do_joystick(_3do_joystick_report* report, uint32_t buttons)
{
    ...
    report->FIRE = (buttons & JP_BUTTON_L2) != 0 ? 1 : 0;
}
...
void __not_in_flash_func(update_3do_report)(uint8_t player_index) {
  ...
  if (output_mode == TDO_MODE_SILLY) {
    ...
    if (player_index == 0) {
      report.p1_coin = (mapped.buttons & JP_BUTTON_S1) != 0 ? 1 : 0;   // Select = P1 Coin
      report.p1_start = (mapped.buttons & JP_BUTTON_S2) != 0 ? 1 : 0;  // Start = P1 Start
      report.p2_coin = (mapped.buttons & JP_BUTTON_L1) != 0 ? 1 : 0;   // L1 = P2 Coin
      report.p2_start = (mapped.buttons & JP_BUTTON_R1) != 0 ? 1 : 0;  // R1 = P2 Start
      // Service = L2 + R2 together
      bool l2_pressed = (mapped.buttons & JP_BUTTON_L2) != 0;
      bool r2_pressed = (mapped.buttons & JP_BUTTON_R2) != 0;
  ...
  if (is_joystick) {
    ...
    // USB active-low: 0 = pressed, 3DO active-HIGH: 1 = pressed
    report.left = (mapped.buttons & JP_BUTTON_DL) != 0 ? 1 : 0;
    report.right = (mapped.buttons & JP_BUTTON_DR) != 0 ? 1 : 0;
    report.up = (mapped.buttons & JP_BUTTON_DU) != 0 ? 1 : 0;
    report.down = (mapped.buttons & JP_BUTTON_DD) != 0 ? 1 : 0;
